...
class SalesView(MasterView):
    ...
    def save_receipt(self):
        ...
        customer_code = data.get('customer', '')  # data['customer']
        seller = data.get('seller', '')  # data['seller']
        products = data.get('products', '')  # data['products']
        observations = data.get('observations', '')  # data['observations']
        ...
        try:
            ...
            document.save()
            # return set_response(None, 404, "Ocurrió un error al grabar el comprobante")
        ...
        set nocount on; EXEC sp_web_V_MV_CPTE '{customer_code}','{seller}','{date}','{observations}','0','0','{tc}',@pRes OUTPUT, @pMensaje OUTPUT,@pIdCpte OUTPUT
        ...
        if response_code == 11:
            for item in products:
                ...
                EXEC sp_web_CpteInsumos {receipt_id},'{item['code']}',{item['quantity']},{item['price']},{item['discount']},@pRes OUTPUT, @pMensaje OUTPUT,@pIdCpte OUTPUT

                SELECT @pRes as pRes, @pMensaje as pMensaje, @pIdCpte pIdCpte
                """

                try:
                    product_result, error = exec_customer_sql(
                        sql, f".No se pudo grabar el producto. \n {sql}", self.token_global)
                ...
                if error:
                    self.log(str(result[0]['message']) +
                             "\nSENTENCIA : " + sql)
                    return set_response(None, 404, "Ocurrió un error al grabar el producto")
        else:
            self.log(response_message)
            return set_response(None, 404, response_message)

        if name != "" or email != "" or phone != "":

            sql = f"""
            UPDATE V_MV_CPTE SET NOMBRE = '{name}', SOLICITANTE = '{email}', TELEFONO = '{phone}' WHERE ID = {receipt_id}
            """

            try:
                result, error = exec_customer_sql(sql, "", self.token_global, False)
            except Exception as e:
                error = True

            if error:
                self.log(str(result[0]['message']) + "\nSENTENCIA : " + sql)
                return set_response(None, 404, "Ocurrió un error al grabar el comprobante. Datos extra.")

        self.log(
            f"Pedido grabado correctamente : ID: {receipt_id} | Cliente: {customer} | Nombre: {name} | Email: {email} | Tel: {phone} | Vendedor: {seller} | Fecha: {date} | Observaciones: {observations}\nProductos: {products}", "INFO")
        # return set_response(None, 200, "Comprobante creado con éxito")
        return set_response([{'receipt_id': receipt_id}], 200, "Comprobante creado con éxito")
    ...
    def save_payments(self):
        ...
        """
        1. Dar de alta la cobranza en V_MV_CPTE
        2. Generar el asiento con los medios de pago
        """

        # Creo la cobranza en V_MV_CPTE
        sql = f"""
        DECLARE @pRes INT
        DECLARE @pMensaje NVARCHAR(250)
        DECLARE @pIdCpte INT
        ...
        self.generate_first_record(payment_id)
        ...
        for payment in payments:
            ...
            EXEC sp_web_creaLineaAsiento {payment_id},{payment['amount']},'{payment['code']}',0,Null,@pResultado OUTPUT,@pMensaje OUTPUT
