...
class CashBoxView(MasterView):
    ...
    def get_banks(self):
        ...
        """
        response = self.get_response(query, f"Ocurrió un error al obtener los bancos", True)
    ...
    def set_movement(self):
        ...
        if error:
            ...
            return set_response(None, 404, "Ocurrió un error al generar el movimiento de caja.")
    ...
    def get_imputation_accounts(self):
        """
        Retorna las cuentas de imputación de caja
        """
        query = f"""
        SELECT codigo,descripcion FROM MA_CUENTAS WHERE (MEDIODEPAGO='' or MEDIODEPAGO IS NULL) and titulo = 0 AND CAJAYBANCO = 1
        """
        response = self.get_response(query, f"Ocurrió un error al obtener las cuentas de imputación", True)
