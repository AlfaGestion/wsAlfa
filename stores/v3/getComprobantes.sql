DECLARE @cuenta nvarchar(9),
	...
	IF @tc <> ''
		SET @WHERE = ' a.TC =''' + @tc + ''''
	ELSE
		SET @WHERE = ' (a.TC =''FC'' OR a.TC =''FP'' OR a.TC=''NP'') '

	IF @cuenta <> ''
		SET @WHERE = @WHERE + ' AND a.Cuenta =''' + @cuenta + ''''
	...
	IF @status <> ''
		BEGIN
			IF @status = 'P' SET @WHERE = @WHERE + ' AND (a.anulada = 0 AND a.finalizada = 0 AND a.bloqueada = 0) '
			IF @status = 'A' SET @WHERE = @WHERE + ' AND (a.anulada = 1 ) '
			IF @status = 'F' SET @WHERE = @WHERE + ' AND (a.finalizada = 1 ) '
			IF @status = 'B' SET @WHERE = @WHERE + ' AND (a.bloqueada = 1 ) '
			IF @status = 'R' SET @WHERE = @WHERE + ' AND (not a.fechapreparacion is null) '
		END
